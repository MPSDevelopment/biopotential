...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			LOGGER.info("start");
			// read input *.wav file, convert to AudioInputStream -> get list of
			// double -> summarize, get ChunkSummary
			...
			// next step compare sample with all desease patterns from db and
			// take with filter Max
			 ...
			 * PatternDB iter = db.getDiseases(); while (true) { Pattern strain
			 * = iter.next(); if (strain == null) { break; } AnalysisSummary
			 * comparison = Analyzer.compare(sample, strain.getSummary()); if
			 * (comparison.getDispersion() == 0) { diseases.put(strain,
			 * comparison); } }
			 */
			...
			diseases.forEach((dk, dv) -> {
				...
				if (probableKinds.containsKey(dk.getKind())) {
					final Map<Pattern, AnalysisSummary> healings = Machine.summarizePatterns(sample, db.getIterForFolder(((EDXPattern) dk).getCorrectingFolder())); // вытягиваются
																																									// папка
																																									// с
																																									// коректорами
																																									// для
