...
/** Creates an instance of OpenALInfo *//*
    ...
    public static void main(String[] args) {
        ALCdevice device = alcOpenDevice(args.length == 0 ? null : args[0]);

        ALCCapabilities deviceCaps = ALC.getCapabilities();

        ALCcontext alContext = alcCreateContext(device, (IntBuffer)null);
//        checkALCError(device);

        alcMakeContextCurrent(alContext);
        ALCCapabilities contextCaps = ALC.getCapabilities();
    ...
    private static void printALCInfo(ALCdevice device, ALCCapabilities caps) {
        ...
        if ( caps.ALC_ENUMERATE_ALL_EXT ) {
            System.out.println("Default playback device: " + alcGetString(device, EnumerateAllExt.ALC_DEFAULT_ALL_DEVICES_SPECIFIER));
        } else {
            System.out.println("Default playback device: " + alcGetString(device, ALC_DEFAULT_DEVICE_SPECIFIER));
        }

        System.out.println("Default capture device: " + alcGetString(device, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER));
...
//        System.out.println("ALC version: " + majorVersion + "." + minorVersion);
    ...
    private static void printEFXInfo(ALCdevice device) {
