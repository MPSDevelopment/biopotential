package com.mpsdevelopment.biopotential.server.wave;// Wav file IO class
// A.Greensted
...
public class WaveFile
{
	...
	// Cannot instantiate WaveFile directly, must either use newWavFile() or openWavFile()
	private WaveFile()
	{
	...
	public static WaveFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and initialise
		WaveFile waveFile = new WaveFile();
		waveFile.file = file;
		waveFile.numChannels = numChannels;
		waveFile.numFrames = numFrames;
		waveFile.sampleRate = sampleRate;
		waveFile.bytesPerSample = (validBits + 7) / 8;
		waveFile.blockAlign = waveFile.bytesPerSample * numChannels;
		waveFile.validBits = validBits;
		...
		waveFile.oStream = new FileOutputStream(file);

		// Calculate the chunk sizes
		long dataChunkSize = waveFile.blockAlign * numFrames;
		...
		if (dataChunkSize % 2 == 1) {
			mainChunkSize += 1;
			waveFile.wordAlignAdjust = true;
		}
		else {
			waveFile.wordAlignAdjust = false;
		}

		// Set the main chunk size
		putLE(RIFF_CHUNK_ID,	waveFile.buffer, 0, 4);
		putLE(mainChunkSize,	waveFile.buffer, 4, 4);
		putLE(RIFF_TYPE_ID,	waveFile.buffer, 8, 4);

		// Write out the header
		waveFile.oStream.write(waveFile.buffer, 0, 12);

		// Put format data in buffer
		long averageBytesPerSecond = sampleRate * waveFile.blockAlign;

		putLE(FMT_CHUNK_ID,				waveFile.buffer, 0, 4);		// Chunk ID
		putLE(16,							waveFile.buffer, 4, 4);		// Chunk Data Size
		putLE(1,								waveFile.buffer, 8, 2);		// Compression Code (Uncompressed)
		putLE(numChannels,				waveFile.buffer, 10, 2);		// Number of channels
		putLE(sampleRate,					waveFile.buffer, 12, 4);		// Sample Rate
		putLE(averageBytesPerSecond,	waveFile.buffer, 16, 4);		// Average Bytes Per Second
		putLE(waveFile.blockAlign,		waveFile.buffer, 20, 2);		// Block Align
		putLE(validBits,					waveFile.buffer, 22, 2);		// Valid Bits

		// Write Format Chunk
		waveFile.oStream.write(waveFile.buffer, 0, 24);

		// Start Data Chunk
		putLE(DATA_CHUNK_ID,				waveFile.buffer, 0, 4);		// Chunk ID
		putLE(dataChunkSize,				waveFile.buffer, 4, 4);		// Chunk Data Size

		// Write Format Chunk
		waveFile.oStream.write(waveFile.buffer, 0, 8);

		// Calculate the scaling factor for converting to a normalised double
		if (waveFile.validBits > 8)
		{
			// If more than 8 validBits, data is signed
			// Conversion required multiplying by magnitude of max positive value
			waveFile.floatOffset = 0;
			waveFile.floatScale = Long.MAX_VALUE >> (64 - waveFile.validBits);
		}
		else
		{
			// Else if 8 or less validBits, data is unsigned
			// Conversion required dividing by max positive value
			waveFile.floatOffset = 1;
			waveFile.floatScale = 0.5 * ((1 << waveFile.validBits) - 1);
		}

		// Finally, set the IO State
		waveFile.bufferPointer = 0;
		waveFile.bytesRead = 0;
		waveFile.frameCounter = 0;
		waveFile.ioState = IOState.WRITING;

		return waveFile;
	}

	public static WaveFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		waveFile.iStream = new FileInputStream(file);

		// Read the first 12 bytes of the file
		int bytesRead = waveFile.iStream.read(waveFile.buffer, 0, 12);
		if (bytesRead != 12) throw new WavFileException("Not enough wav file bytes for header");

		// Extract parts from the header
		long riffChunkID = getLE(waveFile.buffer, 0, 4);
		long chunkSize = getLE(waveFile.buffer, 4, 4);
		long riffTypeID = getLE(waveFile.buffer, 8, 4);
		...
		while (true)
		{
			...
			bytesRead = waveFile.iStream.read(waveFile.buffer, 0, 8);
			...
			long chunkID = getLE(waveFile.buffer, 0, 4);
			chunkSize = getLE(waveFile.buffer, 4, 4);
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				bytesRead = waveFile.iStream.read(waveFile.buffer, 0, 16);

				// Check this is uncompressed data
				int compressionCode = (int) getLE(waveFile.buffer, 0, 2);
				if (compressionCode != 1) throw new WavFileException("Compression Code " + compressionCode + " not supported");

				// Extract the format information
				waveFile.numChannels = (int) getLE(waveFile.buffer, 2, 2);
				waveFile.sampleRate = getLE(waveFile.buffer, 4, 4);
				waveFile.blockAlign = (int) getLE(waveFile.buffer, 12, 2);
				waveFile.validBits = (int) getLE(waveFile.buffer, 14, 2);

				if (waveFile.numChannels == 0) throw new WavFileException("Number of channels specified in header is equal to zero");
				if (waveFile.blockAlign == 0) throw new WavFileException("Block Align specified in header is equal to zero");
				if (waveFile.validBits < 2) throw new WavFileException("Valid Bits specified in header is less than 2");
				if (waveFile.validBits > 64) throw new WavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");

				// Calculate the number of bytes required to hold 1 sample
				waveFile.bytesPerSample = (waveFile.validBits + 7) / 8;
				if (waveFile.bytesPerSample * waveFile.numChannels != waveFile.blockAlign)
				...
				numChunkBytes -= 16;
				if (numChunkBytes > 0) waveFile.iStream.skip(numChunkBytes);
			}
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (chunkSize % waveFile.blockAlign != 0) throw new WavFileException("Data Chunk size is not multiple of Block Align");

				// Calculate the number of frames
				waveFile.numFrames = chunkSize / waveFile.blockAlign;
			...
			else
			{
				...
				waveFile.iStream.skip(numChunkBytes);
			}
		}
		...
		if (waveFile.validBits > 8)
		{
			...
			waveFile.floatOffset = 0;
			waveFile.floatScale = 1 << (waveFile.validBits - 1);
		}
		else
		{
			// Else if 8 or less validBits, data is unsigned
		...
		waveFile.bytesRead = 0;
		waveFile.frameCounter = 0;
		waveFile.ioState = IOState.READING;
	...
	{
		if (ioState != IOState.READING) throw new IOException("Cannot read from WaveFile instance");
	...
	{
		if (ioState != IOState.WRITING) throw new IOException("Cannot write to WaveFile instance");
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				WaveFile readWaveFile = openWavFile(new File(filename));
				readWaveFile.display();

				long numFrames = readWaveFile.getNumFrames();
				int numChannels = readWaveFile.getNumChannels();
				int validBits = readWaveFile.getValidBits();
				long sampleRate = readWaveFile.getSampleRate();

				WaveFile writeWaveFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
				...
				do
				{
					framesRead = readWaveFile.readFrames(buffer, BUF_SIZE);
					framesWritten = writeWaveFile.writeFrames(buffer, BUF_SIZE);
			...
			WaveFile writeWaveFile = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
			double[] buffer = new double[10];
			writeWaveFile.writeFrames(buffer, 10);
			writeWaveFile.close();
		}
		catch (Exception e)
		{
