		...
		diseases.forEach(new BiConsumer<Pattern, AnalysisSummary>() {
			...
			public void accept(Pattern dk, AnalysisSummary dv) {
				...
				if (probableKinds.containsKey(dk.getKind())) {
					final Map<Pattern, AnalysisSummary> healings = Machine.summarizePatterns(new SummaryCondition() {
						/**
						 * и потом берутся только те которые
						 * summary.getDispersion() == 0 т.е. MAx
						 */
						@Override
						public boolean test(Pattern pattern, AnalysisSummary summary) { //
							return summary.getDegree() == 0;
						}
					}, sample, db.getIterForFolder(((EDXPattern) dk).getCorrectingFolder())); // вытягиваются
																								// папка
																								// с
																								// коректорами
																								// для
